...
export const getAsteroidData = async (id: string) => {
  // Simulate API delay
...
export const simulateAsteroid = async (params: AsteroidParams): Promise<SimulationResults> => {
  ...
  const craterSize = Math.pow(kineticEnergy / 1e15, 0.25) * 2; // rough approximation in km
  ...
  const baseCasualties = Math.pow(size, 1.5) * Math.pow(velocity, 0.5) * 100;
...
export const testMitigation = async (
  method: string, 
